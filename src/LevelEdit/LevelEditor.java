...
public class LevelEditor {
	...
	public static void level_editor() {
		...
		/*Start.pane = new JPanel() {
	     ...
	     Start.frame.getContentPane().add(Start.pane); 
	...
	public static void draw_grid(Graphics g) {
		...
		int x_offset = (int)(temp.x % Camera.pixel_size), y_offset = (int)(temp.y % Camera.pixel_size);
		...
		for (int x = - Start.pane.getWidth() / grid_size / 2; x<Start.pane.getWidth() / grid_size / 2; x++) {
			for (int y = - Start.pane.getHeight() / grid_size / 2; y<Start.pane.getHeight() / grid_size / 2; y++) {
				g.drawRect((int)temp.x + x * grid_size + grid_size / 2, (int)temp.y + y * grid_size - y_offset, grid_size, grid_size);
			}
		}
	}
	public static void add_mouse_functions() {
		...
		//if ()
				...
				if (Start.o_pane != null) {
					UI.OptionPane current_pane = Start.o_pane;
					System.out.println(Start.o_pane);
					for (int x = 0; x<current_pane.buttons.length; x++) {
						current_pane.buttons[x].button_clicked(click);
					}
					return;
				}
