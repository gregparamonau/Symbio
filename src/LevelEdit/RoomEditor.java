...
public class RoomEditor {	
	...
	public static int last_object_selected, last_node_selected;
	
	public static String object_type = "GameObject";
	public static String current_area = "mouth";
	
	public static String[] object_names = {"GameObject", "EnemySpawner", "Acid", "BezierTerrain", "Bouncer", "Circle", "Mover", "OneWay", "Slope", "SBTerrain", "Sprite"};
	...
	public static void update() {
		...
		//System.out.println("EDITING ROOM: " + room_editing);
		
	}
	
	public static void draw_view(Graphics g) {
		try {
			room.draw(g, Start.pane, LevelEditor.pos.x, LevelEditor.pos.y, "edit", false);
	...
	public static void mouse_pressed_function(MouseEvent e) {
		...
		last_node_selected = object_select[1];
	}
	...
	public static void display_options() {
		...
		System.out.println("last_object_selected at display_options: " + last_object_selected);
		...
		Start.o_pane = new OptionPane(LevelEditor.click_save, list, new Runnable[] {
				() -> {
					...
					else if (last_object_selected != -1) show_object_properties_panel();
				},
				() -> {
					add_object();
					object_select = new int[] {-1, -1};
	...
	public static void show_object_properties_panel() {
		...
		in_managing_menu = room.objects[last_object_selected].properties(last_node_selected);
	...
	public static void find_select() {
		...
		for (int x = 0; x<room.objects.length; x++) {
			for (int y = 0; y<room.objects[x].nodes.length; y++) {
				if (Vector2.dist(room.objects[x].nodes[y], LevelEditor.click) < Vector2.radius) {
	...
	/*public static void delete_object(int place) {
	    ...
	    for (int x = 0; x < temp.length; x++) {
	        ...
	        temp[x] = room.objects[source_index];
	    }
	    room.objects = temp;
	}*/
	public static void delete_object(int place) {
		...
		for (int x = 0; x<temp.length; x++) {
			int a = 1;
			temp[x] = room.objects[x >= place ? x + a : x];
		}
		room.objects = temp;
	}
