...
public class Line {
	...
	public static Vector2 disp_rect(Line ln, Rectangle in, int direction) {
		...
		double min_dot = Vector2.ndot(Vector2.sub(nodes[0], p), normal);
		
		for (int x = 1; x<nodes.length; x++) {
			
			double d = Vector2.ndot(Vector2.sub(nodes[x], p), normal);
			
			if (d < min_dot) min_dot = d;
		}
		
		if (min_dot > 0) return Vector2.zero;
		//normal.mult(-1);
		
		Vector2 corr = Vector2.mult(normal, -min_dot);
	...
	public static Vector2 find_node_on_line(Line a, Vector2 in) {
		...
		Vector2 p = Vector2.add(a.a, ac._mult(clamp(Vector2.dot(ac, ab) / ac.l() / ac.l(), 0, 1)));
