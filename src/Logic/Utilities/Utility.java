...
public class Utility {
	...
	public static double[] add_to_array(double[] in, double add) {
	...
	public static double[] append(double[] in, double[] add) {
		
		//System.out.print("APPENDING ");
		double[] out = new double[in.length + add.length];
		
		for (int x = 0; x<out.length; x++) {
			out[x] = (x < in.length ? in[x] : add[x - in.length]);
	...
	public static void remove_duplicates(double[] in, double tolerance) {
		double[] out = new double[] {in[0]};
		
		for (int x = 1; x<in.length; x++) {
			
			if (!in_arr(in[x], out, tolerance)) {
				//System.
				out = add_to_array(out, in[x]);
			}
		}
		in = out;
	}
	
	public static boolean in_arr(double a, double[] in, double tolerance) {
		for (int x = 0; x<in.length; x++) {
			if (Math.abs(in[x] - a) <= tolerance) return true;
		}
		return false;
	}
	//smallest to largest
	//bubble sort
	public static void sort(double[] in) {
		boolean done = true;
		do {
			done = true;
