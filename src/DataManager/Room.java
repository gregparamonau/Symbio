...
import Logic.Utilities.ArrayHandler;
...
public class Room extends Rectangle{
	...
	public Rectangle max_bounds;
	
	//softbodies array (array of pointers to spftbodies in objects)
	//polygon array
	...
	public void start() {
		...
		Game.player.combat.respawn_set = false;
		
		
		this.generate_terrain_sprites("/object_textures/alina_colors.png");
	...
	public void generate_terrain_sprites(String in) {
		...
		BufferedImage[] terr = new BufferedImage[this.objects.length + 1];
		Graphics [] gs = new Graphics[this.objects.length + 1];
		terr[0] = new BufferedImage((int)this.max_bounds.width + 2 * bump_depth, (int)this.max_bounds.height + 2 * bump_depth, BufferedImage.TYPE_INT_ARGB);
		gs[0] = terr[0].getGraphics();
		gs[0].setColor(new Color(cols[5]));
		
		for (int x = 1; x<terr.length; x++) {
			if (this.objects[x - 1] instanceof Mover) {
				terr[x] = new BufferedImage((int)this.max_bounds.width + 2 * bump_depth, (int)this.max_bounds.height + 2 * bump_depth, BufferedImage.TYPE_INT_ARGB);
				gs[x] = terr[x].getGraphics();
				gs[x].setColor(new Color(cols[5]));
			}
		}
		...
		for (int x = 0; x<this.objects.length; x++) {
			...
			if (this.objects[x].object_handle != -1) this.objects[x].draw_blank_object(gs[this.objects[x].object_handle + 1], new Rectangle(this.max_bounds.pos.x, this.max_bounds.pos.y, this.max_bounds.width + 2 * bump_depth, this.max_bounds.height + 2 * bump_depth));
			else this.objects[x].draw_blank_object(gs[0], new Rectangle(this.max_bounds.pos.x, this.max_bounds.pos.y, this.max_bounds.width + 2 * bump_depth, this.max_bounds.height + 2 * bump_depth));
		}
		
		for (int j = 0; j<terr.length; j++) {
			if (terr[j] == null) continue;
			
			int[] pixels = ((DataBufferInt) terr[j].getRaster().getDataBuffer()).getData(); 
			int w = (int)terr[j].getWidth() + shear_offset, h = (int)terr[j].getHeight(), l = pixels.length;
			
			gs[j].setColor(new Color(cols[5]));
			
			double chance_bump = 0.15;
			
			for (int y = 0; y < bump_depth; y++) {
				for (int x = 0; x < pixels.length; x++) {
					if (pixels[x] == 0) continue;
					
					boolean a = x + w < l, b = x - w >= 0, c = x + 1 < l, d = x - 1 >= 0;
					...
					if (a && pixels[x + w] == 0 ||
						...
						if (r < chance_bump) {
							if (a) pixels[x + w] = cols[5];
							if (b) pixels[x - w] = cols[5];
							if (c) pixels[x + 1] = cols[5];
							if (d) pixels[x - 1] = cols[5];
							//if ()
							//g.fillOval(x % w, x / w, bump_r, bump_r);
						}
					}
			...
			double chance_sub = 0.5, chance_sub_empty = 0.9, chance_cluster = 1.2e-2;
			
			
			for (int col = 1; col < 5; col++) {
				int color_to_paint = cols[col], painter_color = cols[col - 1], rare_color = cols[clamp(col + 1, 0, 4)];
				for (int x = 0; x<pixels.length; x++) {
					if (painter_color == 0 && pixels[x] == background_col && Math.random() < chance_cluster) {
						pixels[x] = cols[1];
						continue;
					}
		...
		int w = (int)terr.getWidth() + shear_offset, h = (int)terr.getHeight(), l = pixels.length;
		...
		//this line is responsible for transparency in the texture, so I prefer to remove it I think
		...
		for (int x = 0; x < this.objects.length; x++) {
		    ...
		    int sample_terr = this.objects[x].object_handle == -1 ? 0 : this.objects[x].object_handle + 1;
		    
		    // terrain image origin in world space
		    double terrLeft = this.max_bounds.pos.x - this.max_bounds.width / 2.0 - bump_depth;
		    double terrTop = this.max_bounds.pos.y + this.max_bounds.height / 2.0 + bump_depth;

		    // object center in image space
		    double cx = o.pos.x - terrLeft;
		    double cy = terrTop - o.pos.y;
		    ...
		    int sx = (int)Math.floor(cx - o.width  / 2.0) - bump_depth;
		    int sy = (int)Math.floor(cy - o.height / 2.0) - bump_depth;

		    int sw = (int)Math.ceil(o.width  + 2 * bump_depth);
		    int sh = (int)Math.ceil(o.height + 2 * bump_depth);
		    /*
...
*/
		    ...
		    int ix1 = Math.min(terr[sample_terr].getWidth(),  sx + sw);
		    int iy1 = Math.min(terr[sample_terr].getHeight(), sy + sh);
		    ...
		    this.objects[x].sprite = terr[sample_terr].getSubimage(ix0, iy0, cw, ch);
		}
		/*for (int x = 0; x<this.objects.length; x++) {
			if (!this.objects[x].terrain) continue;
	...
	public void bounds() {
		Rectangle out = new Rectangle(this);
		
		for (int x = 0; x<this.objects.length; x++) {
			out.expand(this.objects[x]);
		}
	...
	public void start_objects() {
		for (int x = 0; x<this.objects.length; x++) {
			this.objects[x].start();
		}
	}
