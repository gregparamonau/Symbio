...
public class Mover extends GameObject{
	...
	public void drag_player() {
		...
		System.out.println("MOVED DRAG: " + Vector2.sub(Game.player.pos, temp));
	}
	...
	public void draw_object(Graphics g, JPanel pane, double xin, double yin, String location) {
		...
		if (location.equals("game")) {
			...
			this.pol.bounds.draw(g, pane, xin, yin, location, start);
		...
		if (location.equals("edit")) {
			...
			for (int x = 0; x<this.nodes.length - 2; x++) {
				new Line(this.nodes[x + 2], this.nodes[(x + 1) % (this.nodes.length - 2) + 2]).draw_line(g, fill, pane, xin, yin, location, true);
			}
			new Line(this.pos, this.nodes[2]).draw_line(g, fill, pane, xin, yin, location, true);
		}		
	}
