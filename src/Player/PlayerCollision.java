...
public class PlayerCollision {
	...
	public void update_counters() {
		...
		if (!this.col_up && (this.col_left || this.col_right)) this.wall_coyote = coyote_max;
	...
	public void displace(int direction) {
		Vector2 final_disp = new Vector2();
		int count = 0;
		for (int x = 0; x<Game.current_room.objects.length; x++) {
			Vector2 new_disp = Game.current_room.objects[x].displace_entity(this.player, direction);
			if (new_disp.equals(Vector2.zero)) continue;
			
			count++;
			final_disp.add(new_disp);
			/*if (Utility.sign(final_disp.x) != Utility.sign(new_disp.x) || Utility.sign(final_disp.y) != Utility.sign(new_disp.y)) {
				this.player.combat.hazard_respawn(true);
				break;
			}*/
			
			//final_disp.max_mag(new_disp);
		}
		if (count == 0) return;
		
		final_disp.mult(1.0 / count);
		this.player.pos.add(final_disp);
	}
	
	//COLLISION
	public boolean collide(Vector2 move, boolean find_collide) {
		this.player.object_intersect_id = -1;
		//System.out.println("START");
		for (int x = 0; x<Game.current_room.objects.length; x++) {
			boolean sb = Game.current_room.objects[x].sb != null;
			if (!Game.current_room.objects[x].solid && !(sb && find_collide)) continue;
			
			//System.out.println("	COLLIDE_CHECK_393: " + Game.current_room.objects[x].toString());
			if (Game.current_room.objects[x].collide_with(new Rectangle(this.player.pos.x + move.x, this.player.pos.y + move.y, this.player.width + (sb ? 2 : 0), this.player.height + (sb ? 2 : 0)), false)) {
	...
	public boolean object_action() {
		...
		for (int x = 0; x<Game.current_room.objects.length; x++) {
			if (Game.current_room.objects[x].collide_with(new Rectangle(this.player.pos.x, this.player.pos.y, this.player.width + 2, this.player.height + 2), true)) {
